...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchUserData = async (userId: string) => {
    try {
      ...
      if (!roleData) {
        const missingRoleMessage = "Your account has no role assigned. Please contact the administrator.";
        console.error("No role found for user:", userId);
        setAuthError(missingRoleMessage);
        setRole(null);
        setProfile(null);
      ...
      if (userRole === "student") {
        ...
        setDepartmentId((currentDepartmentId) => currentDepartmentId || studentData?.department_id || null);

        setProfile((currentProfile) => {
          const hasRealName =
            !!currentProfile?.full_name &&
            currentProfile.full_name.trim().length > 0 &&
          ...
          return {
            full_name: studentName || currentProfile?.full_name || "Student",
