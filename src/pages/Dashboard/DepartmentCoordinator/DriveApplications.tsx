    ...
    if (error) {
      ...
      if (targetApp?.student_id) {
        if (status === "approved_by_hod") {
          const { data: tpoRoles } = await supabase
            .from("user_roles")
            .select("user_id")
            .eq("role", "placement_officer");

          const recipients = (tpoRoles || []).map((row) => row.user_id).filter(Boolean);
          const studentName = [targetApp.student?.first_name, targetApp.student?.last_name].filter(Boolean).join(" ").trim() || "A student";
          ...
          if (recipients.length) {
            await supabase.from("notifications" as any).insert(
              recipients.map((recipientId) => ({
                ...
                message: `${studentName}${targetApp.student?.reg_no ? ` (${targetApp.student.reg_no})` : ""} was approved by HOD for ${companyName} - ${roleName}. Open Pending Approvals to review the application.`,
          ...
          await supabase.from("notifications" as any).insert({
            ...
            message: `Your application for ${companyName} - ${roleName} has been approved by HOD and forwarded to TPO for final review.`,
            drive_id: targetApp.drive_id,
          });
          ...
          await supabase.from("notifications" as any).insert({
            ...
            message: `Your application for ${companyName} - ${roleName} was rejected by HOD. Please contact your department coordinator if you need clarification.`,
            drive_id: targetApp.drive_id,
          });
        }
      }
