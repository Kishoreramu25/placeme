...
export default function StudentApprovals() {
  ...
  const handleAction = async (studentId: string, status: "approved_by_hod" | "rejected") => {
    ...
    try {
      ...
      if (status === "approved_by_hod" && targetStudent) {
        const { data: tpoRoleRows } = await supabase
          .from("user_roles")
          .select("user_id")
          .eq("role", "placement_officer");

        const tpoIds = (tpoRoleRows || []).map((row) => row.user_id).filter(Boolean);

        if (tpoIds.length) {
          const studentName = [targetStudent.first_name, targetStudent.last_name].filter(Boolean).join(" ").trim() || "A student";
          await supabase.from("notifications" as any).insert(
            tpoIds.map((recipientId) => ({
              user_id: recipientId,
              role: "placement_officer",
              type: "student_verification",
              title: "Student Ready For Final Verification",
              message: `${studentName}${targetStudent.reg_no ? ` (${targetStudent.reg_no})` : ""} was approved by HOD. Open Students Master to complete the final TPO verification.`,
              drive_id: null,
            }))
          );
        }
        ...
        await supabase.from("notifications" as any).insert({
          ...
          role: "student",
          type: "student_verification",
          title: "Profile Approved By HOD",
          message: "Your student profile has been approved by HOD and forwarded to TPO for final verification.",
          drive_id: null,
        });
      }
      ...
      if (status === "rejected") {
        await supabase.from("notifications" as any).insert({
          ...
          role: "student",
          type: "student_verification",
          title: "Profile Rejected By HOD",
          message: "Your student profile was rejected by HOD. Please update the details and resubmit for verification.",
          drive_id: null,
        });
      }
