...
export default function StudentProfile() {
  ...
  }, [user, reset, isLoading, authLoading]);
  ...
  const onActualSubmit = async (data: StudentMasterFormData) => {
    ...
    try {
      ...
      const submittedName = [rest.first_name, rest.last_name].filter(Boolean).join(" ").trim() || "A student";
      const { error } = await supabase.from("students_master").upsert({
        id: user.id,
        ...rest,
        department_id: studentDeptId,
      ...
      if (studentDeptId) {
        ...
        if (hodIds.length) {
          ...
          const recipients = (hodProfiles || []).map((profile) => profile.id).filter(Boolean);
          ...
          if (recipients.length) {
            await supabase.from("notifications" as any).insert(
              recipients.map((recipientId) => ({
                ...
                message: `${submittedName}${rest.reg_no ? ` (${rest.reg_no})` : ""} submitted the student profile for department verification. Open Pending Students to review and approve.`,
                drive_id: null,
              }))
            );
          }
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-20">
      {!hasExistingData && (
        <Alert variant="destructive" className="border-amber-300 bg-amber-50 text-amber-900">
          ...
          <AlertDescription>
            We couldn’t find your saved academic details yet. Please fill in your profile and click Save or Submit.
          </AlertDescription>
        </Alert>
      )}
                ...
                <Card className="border-primary/10 shadow-premium">
                  ...
                  <CardContent className="grid gap-6 md:grid-cols-2 pt-6">
                    ...
                    <div className="space-y-2">
                      ...
                      {errors.current_year && <p className="text-xs text-destructive">{errors.current_year.message}</p>}
                    </div>
