...
import { formatEligibleYearLabel, matchesEligibleYear } from "@/lib/eligibility";
import { 
...
export default function StudentDrives() {
  ...
  const isStudentEligibleForDrive = (studentProfile: any, studentMetrics: any, drive: any) => {
    const eligibleDepartmentIds = (drive.drive_eligible_departments || []).map((entry: any) => entry.department_id);
    const matchesDepartment =
      eligibleDepartmentIds.length === 0 || eligibleDepartmentIds.includes(studentProfile.department_id);

    if (!matchesDepartment) return false;
    if (drive.min_cgpa !== null && studentMetrics.cgpa < (drive.min_cgpa || 0)) return false;
    ...
    if (drive.min_10th_mark !== null && studentMetrics.mark10 < (drive.min_10th_mark || 0)) return false;
    if (drive.min_12th_mark !== null && studentMetrics.mark12 < (drive.min_12th_mark || 0)) return false;

    if (!matchesEligibleYear(drive.eligible_batches, studentMetrics.year)) return false;
  ...
  const { data: drives, isLoading: drivesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const studentMetrics = {
        ...
        year: studentProfile.current_year?.trim() || "4"
      };
      ...
      const drivePromises = ((allDrives as any) || [])
      .filter((drive: any) => isStudentEligibleForDrive(studentProfile, studentMetrics, drive))
      .map((drive: any) => {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (data?.success === false) {
        ...
        if (departmentId) {
          const { data: hodRoleRows } = await supabase
            .from("user_roles")
            .select("user_id")
            .eq("role", "department_coordinator");

          const hodIds = (hodRoleRows || []).map((row) => row.user_id).filter(Boolean);
          ...
          if (hodIds.length) {
            ...
            const recipients = (hodProfiles || []).map((profileRow) => profileRow.id).filter(Boolean);
            const studentName = [profile?.first_name, profile?.last_name].filter(Boolean).join(" ").trim() || "A student";
            const companyName = selectedDrive?.companies?.name || "the drive";
            const roleName = selectedDrive?.role_offered || "the offered role";
            ...
            if (recipients.length) {
              await supabase.from("notifications" as any).insert(
                recipients.map((recipientId) => ({
                  ...
                  title: "New Drive Application Received",
                  message: `${studentName}${profile?.reg_no ? ` (${profile.reg_no})` : ""} applied for ${companyName} - ${roleName}. Open Drive Applications to review the request.`,
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <Select value={sortOrder} onValueChange={(value: "newest" | "oldest") => setSortOrder(value)}>
          <SelectTrigger className="h-11 w-[180px] bg-white font-bold text-xs border-slate-200">
                      ...
                      <div className="bg-slate-50 p-8 rounded-none border-2 border-dashed border-slate-200 space-y-8">
                        ...
                        <div className="grid gap-12 sm:grid-cols-3">
                          ...
                          <div className="space-y-1 text-center">
                            ...
                            <p className="text-lg font-black text-slate-700">{formatEligibleYearLabel(selectedDrive.eligible_batches || "4th Year")}</p>
