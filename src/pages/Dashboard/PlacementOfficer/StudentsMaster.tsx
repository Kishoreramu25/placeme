...
export default function StudentsMaster() {
  ...
  const handleFinalApprove = async (studentId: string) => {
    ...
    try {
      const targetStudent = students.find((student) => student.id === studentId) || selectedStudent;
      ...
      await supabase.from("notifications" as any).insert({
        ...
        title: "Profile Verified By TPO",
        message: `Your student profile${targetStudent?.reg_no ? ` (${targetStudent.reg_no})` : ""} has been fully verified by TPO. You can now proceed with eligible placement activities.`,
        drive_id: null,
      });
  ...
  const filteredConfigColumns = STUDENT_COLUMNS.filter((col) => {
  ...
  return (
    <div className="space-y-6 pb-20">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="h-11 rounded-xl border-slate-200 bg-white pl-10 font-bold text-slate-700"
                  />
                </div>
              </div>
              <p className="mt-2 text-[11px] font-bold uppercase tracking-wide text-slate-400">
                Showing {filteredConfigColumns.length} of {STUDENT_COLUMNS.length} fields
              </p>
            </div>
          </DialogHeader>

          <div className="flex-1 overflow-y-auto p-8 bg-white">
            {filteredConfigColumns.length === 0 ? (
              <div className="flex min-h-[220px] flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-slate-50 text-center">
                ...
                <p className="text-sm font-black uppercase tracking-wide text-slate-700">No matching fields found</p>
                <p className="mt-1 text-xs font-bold text-slate-400">Try another label or column key.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {filteredConfigColumns.map(col => (
                  <Label 
                    key={col.key}
                    className={cn(
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent className="flex max-h-[92vh] w-[min(96vw,80rem)] max-w-5xl flex-col overflow-hidden rounded-3xl border-primary/20 p-0 shadow-2xl">
          <DialogHeader className="shrink-0 border-b bg-slate-900 p-6 text-white sm:p-8">
          ...
          {selectedStudent && (
            <>
                    ...
                    <div className="space-y-4 md:col-span-2">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="font-bold text-primary">{selectedStudent.reg_no}</p>
                        </div>
                      </div>
                    </div>
                    <div className="rounded-3xl border border-dashed bg-slate-50 p-6 text-center">
                      <p className="mb-1 text-[10px] font-black uppercase tracking-widest text-slate-400">Current Verification Tier</p>
                      <Badge className="h-auto bg-slate-900 px-4 py-1.5 text-sm font-black text-white">
                    ...
                    {STUDENT_COLUMNS.map((col) => {
                      ...
                      return (
                        <div key={col.key} className="space-y-1.5 border-b border-slate-100 pb-3">
                          <div className="mb-1 line-clamp-1 text-[10px] font-bold uppercase tracking-widest text-slate-400">{col.label}</div>
                          <div className={cn(
                            "break-words text-sm font-bold text-slate-800",
                            !val && "italic font-normal text-slate-300"
              ...
              {/* ACTION FOOTER */}
              <div className="flex shrink-0 flex-col gap-3 border-t bg-white p-4 sm:flex-row sm:justify-end sm:p-6">
                <Button variant="outline" onClick={() => setIsDetailsOpen(false)} className="h-12 rounded-xl px-8 font-bold">Close Archive View</Button>
                {selectedStudent.approval_status === "approved_by_hod" && (
                  <Button 
                    className="flex h-12 gap-3 rounded-xl bg-primary px-10 font-black text-white shadow-xl transition-all hover:-translate-y-1 hover:bg-slate-800"
