               ...
               <div className="space-y-1">
                 ...
                 <div className="relative mt-4 max-w-md">
                    ...
                    <Input
                      ...
                      onChange={(event) => setGridSearchTerm(event.target.value)}
                      placeholder="Search fields by label or key..."
                      className="h-11 rounded-2xl border-slate-200 bg-slate-50 pl-10 font-bold text-slate-700"
                    />
                 </div>
               ...
               {CATEGORIES.map(cat => (
                 <TabsContent key={cat.id} value={cat.id} className="m-0 focus-visible:outline-none">
                    <div className="p-8 space-y-8 pb-12">
                      ...
                      {(() => {
                        const normalizedSearch = gridSearchTerm.trim().toLowerCase();
                        const filteredColumns = COLUMN_DEFS.filter((col) => {
                          ...
                          return (
                            ...
                            col.key.toLowerCase().includes(normalizedSearch)
                          );
                        });

                        if (filteredColumns.length === 0) {
                          return (
                            <div className="flex min-h-[180px] flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-slate-50 text-center">
                              <Search className="mb-3 h-8 w-8 text-slate-300" />
                              <p className="text-sm font-black uppercase tracking-wide text-slate-700">No matching fields in {cat.id}</p>
