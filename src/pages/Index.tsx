import { Link } from "react-router-dom";
import { Button } from "@/components/ui/button";
import {
  Carousel,
  CarouselContent,
  CarouselItem,
  CarouselNext,
  CarouselPrevious,
} from "@/components/ui/carousel";

import {
  CheckCircle2,
  ArrowRight,
  GraduationCap,
} from "lucide-react";
import React from "react";

const flyerImages = [
  "/flyers/WhatsApp Image 2026-03-18 at 9.36.31 AM.jpeg",
  "/flyers/WhatsApp Image 2026-03-18 at 9.36.31 AM (1).jpeg",
  "/flyers/WhatsApp Image 2026-03-18 at 9.36.30 AM.jpeg",
  "/flyers/WhatsApp Image 2026-03-18 at 9.36.30 AM (1).jpeg",
  "/flyers/WhatsApp Image 2026-03-18 at 9.36.30 AM (2).jpeg",
  "/flyers/WhatsApp Image 2026-03-18 at 9.36.30 AM (3).jpeg",
];





export default function Index() {
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <header className="sticky top-0 z-50 w-full border-b bg-card/80 backdrop-blur-xl">
        <div className="container flex flex-wrap items-center justify-between gap-3 px-4 py-3 sm:h-16 sm:flex-nowrap sm:px-6 sm:py-0">
          <div className="min-w-0 flex flex-1 items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-orange-500 sm:h-10 sm:w-10">
              <GraduationCap className="h-5 w-5 text-white sm:h-6 sm:w-6" />
            </div>
            <span className="line-clamp-2 text-sm font-bold leading-tight sm:line-clamp-1 sm:text-base lg:text-xl">
              ERODE SENGUNTHAR ENGINEERING COLLEGE
            </span>
          </div>
          <nav className="hidden md:flex items-center gap-6">


          </nav>
          <div className="flex w-full items-center gap-2 sm:w-auto sm:gap-4">
            <Button variant="ghost" asChild className="flex-1 hover:bg-orange-50 hover:text-orange-600 sm:flex-none">
              <Link to="/auth">Sign In</Link>
            </Button>
            <Button asChild className="flex-1 border-0 bg-gradient-to-r from-orange-500 to-amber-500 text-white hover:from-orange-600 hover:to-amber-600 sm:flex-none">
              <Link to="/auth?tab=signup">
                Get Started
                <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
            </Button>
          </div>
        </div>
      </header>

      {/* Hero Section */}
      {/* Hero Content Section (Video + Text) - NOW FIRST */}
      <section className="relative flex min-h-[78vh] items-center justify-center overflow-hidden px-4 py-16 sm:min-h-[85vh] sm:px-6 sm:py-20">
        {/* YouTube Video Background */}
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
          <div className="relative -top-[45%] h-[190%] w-full sm:-top-[70%] sm:h-[240%] lg:-top-[100%] lg:h-[300%]">
            <iframe
              src="https://www.youtube.com/embed/sA2il3PqjQE?autoplay=1&mute=1&controls=0&start=30&loop=1&playlist=sA2il3PqjQE&showinfo=0&rel=0&iv_load_policy=3&modestbranding=1&playsinline=1&enablejsapi=1&vq=hd2160"
              className="h-full w-full object-cover scale-[1.9] sm:scale-[1.65] lg:scale-150"
              title="Hero Background Video"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            />
          </div>
          <div className="absolute inset-0 bg-gradient-to-b from-black/55 via-black/35 to-black/65" />
        </div>

        <div className="container relative z-10 px-0">
          <div className="mx-auto max-w-4xl text-center">
            <div className="mb-6 inline-flex max-w-full items-center rounded-full border border-orange-500/50 bg-black/40 px-3 py-2 text-xs text-yellow-300 backdrop-blur-md shadow-lg sm:mb-8 sm:px-4 sm:text-sm">
              <CheckCircle2 className="mr-2 h-4 w-4 shrink-0 text-orange-400" />
              Trusted by leading institutions
            </div>
            <h1 className="mb-5 text-3xl font-extrabold tracking-tight text-white drop-shadow-[0_4px_4px_rgba(0,0,0,0.8)] sm:mb-6 sm:text-5xl md:text-6xl lg:text-7xl">
              <span className="bg-gradient-to-r from-orange-400 to-amber-300 bg-clip-text text-transparent drop-shadow-none filter brightness-110">
                ERODE SENGUNTHAR ENGINEERING COLLEGE
              </span>
            </h1>
            <p className="mx-auto mb-8 max-w-2xl text-base font-bold leading-relaxed text-white drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)] sm:mb-10 sm:text-lg md:text-2xl">
              Erode Sengunthar Engineering College is accredited by National Board of Accreditation (NBA), approved by AICTE and Permanently affiliated to Anna University.
            </p>
            <div className="flex flex-col items-center justify-center gap-4 sm:flex-row">
              <Button size="lg" asChild className="h-auto w-full border border-orange-400/20 bg-gradient-to-r from-orange-600 to-amber-600 px-6 py-4 text-base text-white shadow-xl hover:from-orange-700 hover:to-amber-700 sm:w-auto sm:px-8 sm:py-6 sm:text-lg">
                <Link to="/auth?tab=signup">
                  ADMISSIONS OPEN
                  <ArrowRight className="ml-2 h-5 w-5 sm:h-6 sm:w-6" />
                </Link>
              </Button>

            </div>
          </div>
        </div>

        {/* Scroll Indicator */}
        <div className="absolute bottom-6 left-1/2 hidden -translate-x-1/2 animate-bounce text-white/80 drop-shadow-md sm:block">
          <ArrowRight className="h-8 w-8 rotate-90" />
        </div>
      </section>

      {/* Placement Record 2026 Section */}
      <section className="w-full">
        <img
          src="/placement_banner_2026.jpg"
          alt="Erode Sengunthar Engineering College Placement Record 2026"
          className="w-full h-auto object-cover"
        />
      </section>

      <section className="w-full px-4 py-6 sm:px-6 sm:py-8">
        <div className="mx-auto max-w-5xl overflow-hidden rounded-2xl shadow-lg">
          <img
            src="/flyers/esec_orgchart_exact.jpg.jpeg"
            alt="Erode Sengunthar Engineering College organizational chart"
            className="w-full h-auto object-contain"
          />
        </div>
      </section>

      {/* Hero Image Section - Full Screen Width */}
      <section className="w-full">
        <img
          src="https://erode-sengunthar.ac.in/wp-content/uploads/2025/05/WEB-POSTER-scaled.jpg"
          alt="Erode Sengunthar Engineering College Poster"
          className="w-full h-auto object-cover"
        />
      </section>

      {/* Flyers Carousel Section */}
      <section className="bg-muted/30 py-12 sm:py-16">
        <div className="container px-4 md:px-6">
          <div className="mb-8 text-center sm:mb-10">
            <h2 className="text-2xl font-bold tracking-tighter sm:text-4xl md:text-5xl">Recent Events & Highlights</h2>
            <p className="mx-auto mt-4 max-w-[700px] text-sm text-muted-foreground sm:text-base md:text-xl">
              Explore our college life and recent placement achievements.
            </p>
          </div>
          <div className="relative mx-auto max-w-5xl px-2 sm:px-12">
            <Carousel
              opts={{
                align: "start",
                loop: true,
              }}
              className="w-full"
            >
              <CarouselContent>
                {flyerImages.map((src, index) => (
                  <CarouselItem key={index} className="basis-[86%] p-2 sm:basis-1/2 lg:basis-1/3">
                    <div className="overflow-hidden rounded-xl border bg-background shadow-lg transition-all hover:scale-[1.02]">
                      <img
                        src={src}
                        alt={`Flyer ${index + 1}`}
                        className="aspect-[3/4] w-full object-cover"
                        loading="lazy"
                      />
                    </div>
                  </CarouselItem>
                ))}
              </CarouselContent>
              <CarouselPrevious className="absolute -left-4 hidden h-11 w-11 border-orange-500 text-orange-600 hover:bg-orange-50 sm:flex lg:-left-12 lg:h-12 lg:w-12" />
              <CarouselNext className="absolute -right-4 hidden h-11 w-11 border-orange-500 text-orange-600 hover:bg-orange-50 sm:flex lg:-right-12 lg:h-12 lg:w-12" />
            </Carousel>
            <p className="mt-4 text-center text-xs text-muted-foreground sm:hidden">
              Swipe to explore more highlights.
            </p>
          </div>
        </div>
      </section>
      {/* Footer - Detailed */}
      <footer className="bg-[#333333] py-12 text-white sm:py-16">
        <div className="container px-4 sm:px-6">
          <div className="grid grid-cols-1 gap-10 md:grid-cols-3 md:gap-12">

            {/* Column 1: Important Links */}
            <div>
              <h3 className="mb-5 text-lg font-bold uppercase text-yellow-400 sm:mb-6 sm:text-xl">ADMISSIONS OPEN</h3>
              <ul className="space-y-3 text-sm text-gray-300">
                <li className="hover:text-yellow-400 cursor-default transition-colors">Admission</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">Departments</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">Placement</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">Controller of Examination</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">Anti Ragging Committee</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">Research</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">Alumni</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">AICTE IDEA Lab</li>
              </ul>
            </div>

            {/* Column 2: Quick Links */}
            <div>
              <h3 className="mb-5 text-lg font-bold uppercase text-yellow-400 sm:mb-6 sm:text-xl">WITH </h3>
              <ul className="space-y-3 text-sm text-gray-300">
                <li className="hover:text-yellow-400 cursor-default transition-colors">NAAC</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">NIRF</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">NBA</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">AICTE- Mandatory Disclosure</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">Audit Statements</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">IIC</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">Institutional Development Plan</li>
                <li className="hover:text-yellow-400 cursor-default transition-colors">Online Payment</li>
              </ul>
            </div>

            {/* Column 3: Contact Us */}
            <div>
              <h3 className="mb-5 text-lg font-bold uppercase text-yellow-400 sm:mb-6 sm:text-xl">CONTACT US</h3>
              <h4 className="mb-2 text-base font-bold text-white sm:text-lg">Erode Sengunthar Engineering College</h4>
              <div className="text-gray-300 text-sm space-y-1 mb-4">
                <p>Thudupathi, Perundurai</p>
                <p>Erode – 638 057, TamilNadu, India</p>
                <p>Mail : <a href="mailto:contact@esec.ac.in" className="hover:text-yellow-400">contact@esec.ac.in</a></p>
                <p>Mobile : 9442132706</p>
              </div>

              {/* Map Embed */}
              <div className="mt-4 h-56 w-full overflow-hidden rounded-md bg-gray-200 sm:h-48">
                <iframe
                  src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d15647.76674466946!2d77.618645!3d11.338576!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3ba96cf0b0b8c541%3A0xe5a3c9a6327e5ec0!2sErode%20Sengunthar%20Engineering%20College!5e0!3m2!1sen!2sin!4v1684300000000!5m2!1sen!2sin"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
                  allowFullScreen={true}
                  loading="lazy"
                  referrerPolicy="no-referrer-when-downgrade"
                  title="College Location"
                ></iframe>
              </div>
            </div>

          </div>

          <div className="mt-10 border-t border-gray-700 pt-8 text-center text-sm text-gray-500 sm:mt-12">
            {/* © Zenetive Infotech — hidden for now */}
          </div>
        </div>
      </footer>
    </div>
  );
}
